...
for sign in signs:
    for example_index in range(num_examples):
        ...
        for frame_index in range(num_frames):
            res = np.load(os.path.join(DATA_DIR, sign, str(example_index), "{}.npy".format(frame_index+1)))
            window.append(res)
...
#model.save(f'{MODEL_DIR}/alphabet_motion_classifier_diff.keras') # save model
